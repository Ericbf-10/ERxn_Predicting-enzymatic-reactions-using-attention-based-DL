...
if RESUME_TRAINING:
    # load model state
    state_dict = os.path.join(results_dir, f'10_protein_autoencoder')
...
for epoch in range(EPOCH, NUM_EPOCHS):
    ...
    if epoch % (1) == 0:
        summary.append('Train Epoch: {}\tLoss: {:.6f}\tTest Loss: {:.6f}'.format(epoch, train_loss[-1], test_loss[-1]))
        print('Train Epoch: {}\tLoss: {:.6f}\tTest Loss: {:.6f}'.format(epoch, train_loss[-1], test_loss[-1]))

    if invoke(early_stopping, test_loss[-1], model, implement=True):
        model.load_state_dict(torch.load(os.path.join(results_dir,'10_protein_autoencoder'), map_location=device))
        summary.append(f'Early stopping after {epoch} epochs')
        break

    torch.save(model.state_dict(), os.path.join(results_dir, f'10_protein_autoencoder'))
...
with torch.no_grad():
    ...
    for i, (x_valid, y_valid, _, _) in enumerate(valid_loader):
        ...
        _, recon = model(x_valid)

        loss = criterion(recon, x_valid)
        valid_loss += loss.data

    summary.append('Valid Loss: {:.6f}'.format(valid_loss))
